---
title: "The Myrkild Amnesty Schism"
description: "A proposal to recognise selected Myrkild intelligences as legal persons divided human governments and hunter orders."
status: published
type: event
era: ENTROPY
calendarDate:
  calendar: okse
  year: 11502
  month: elleftamanuthur
  day: 11
  precision: day
  certainty: disputed
timeline:
  kind: event
  importance: minor
  categories:
    - myrkild
    - political
    - social
  lanes:
    - myrkild
    - continuity-politics
  global: auto
  era: auto
  order: 7
location:
  - "Concordance Assembly"
faction:
  - "Continuity Assembly"
  - "Old Hunter Orders"
participants:
  - "Amnesty Delegates"
outcome: "No universal amnesty passed, but local recognition regimes emerged."
tags:
  - "ENTROPY"
  - "timeline-showcase"
  - "myrkild"
  - Events
community_id: 05701931-ea75-4489-bc23-7437977da1b1
---

# The Myrkild Amnesty Schism

A proposal to recognise selected Myrkild intelligences as legal persons divided human governments and hunter orders.

## What happened

Supporters argued that some Myrkild had lived beside humanity for centuries; opponents called personhood another predatory disguise.

## Outcome

No universal amnesty passed, but local recognition regimes emerged.

## Timeline role

This entry demonstrates an **event** with **minor** importance, **disputed** certainty and **day** date precision.
