...
const fm = ["---", `title: ${JSON.stringify(title)}`, `description: ${JSON.stringify("A WIP Myrkild unit profile.")}`, "created:", "published:", "updated:", "status: draft", "type: myrkild-unit", "unit_id: null", "source_id: null", `unit_name: ${JSON.stringify(title)}`, `era: ${JSON.stringify(era)}`, "species: Myrkild", `myrkild_species: ${JSON.stringify(myrkildSpecies)}`, `origin: ${JSON.stringify(origin)}`, "natural_host: null", "role: null"];
