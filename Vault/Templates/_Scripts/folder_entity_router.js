...
function fillBlankScalar(source, key, value) {
  ...
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\function fillBlankScalar(source, key, value) {
  if (!value) return source;
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return source.replace(new RegExp(`^${escaped}:\\s*$`, "m"), `${key}: ${JSON.stringify(value)}`);
}
");
  return source.replace(new RegExp(`^${escaped}:\\s*const ROUTES = Object.freeze({
  article: { label: "General article", template: "Templates/Lore/Article Template.md" },
  character: { label: "Character", template: "Templates/Lore/Character Template.md" },
  faction: { label: "Faction", template: "Templates/Lore/Faction Template.md" },
  location: { label: "Location", template: "Templates/Lore/Location Template.md" },
...
function insertScalarIfMissing(source, key, value) {
  if (!value) return source;
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\function fillBlankScalar(source, key, value) {
  if (!value) return source;
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
");
  if (new RegExp(`^${escaped}:`, "m").test(source)) return fillBlankScalar(source, key, value);
  ...
  lines.splice(typeIndex >= 0 ? typeIndex + 1 : end, 0, `${key}: ${JSON.stringify(value)}`);
  return lines.join("\n");
}
...
function applyAuthoringBaseline(source, type, folderPath) {
  ...
  if (STUB_TYPES.has(type)) rendered = insertScalarIfMissing(rendered, "development_level", "stub");
  return rendered;
}
