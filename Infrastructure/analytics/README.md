...
Each normal timer run also fills any missing completed monthly periods, starting with August 2026, through the previous London calendar month. Monthly rows are calculated directly from raw events, so monthly unique visitors are not summed from daily values. Automatic monthly catch-up leaves snapshot totals `NULL`.
