...
The same shell provides the VISCERIUM presentation layer without forking Remark42. It preserves Remark42's own colour-token architecture instead of rebinding the numbered `--color*` palette independently inside `.light` and `.dark`. The neutral numbered tokens are defined once on the document root, matching upstream's semantic contract: for example the light composer remains dark `--color0` text on light `--color6`, while the dark composer remains light `--color5` text on dark `--color8`. Only Remark42's named theme variables and era accent tokens vary under its literal direct-child `.light`/`.dark` root classes. CITADEL, SMOG, NEARSIGHT and ENTROPY accents are inferred from the canonical `/eras/<era>/...` article URL. Runtime theme switching otherwise stays stock Remark42: the parent embed owns the iframe `color-scheme` and theme message, and the shell does not add a second dynamic document-level scheme path. Native Remark42 composer, link and button geometry remains upstream-owned; the override recolours those controls rather than restyling them. VISCERIUM-owned structural additions are limited to the era-coloured comment root rail, borderless avatars, compact identity metadata and the Staff role tag.

No persistent `MutationObserver` is installed by the VISCERIUM layer. Remark42's controlled textarea re-renders on every keystroke, so even a child-list-only subtree observer can enter the editor hot path. Initial decoration polls only until the top-level form exists and Remark42's preloaders are gone (or a hard attempt cap is reached), then stops. Later passes are limited to low-frequency actions that can create or reconcile forms/comments (`submit`, click, and Remark42's explicit theme message) and are coalesced through `requestAnimationFrame`. `focusin`, `input` and `keydown` are deliberately not observed. This is a compatibility requirement: ordinary typing must not wake the VISCERIUM decoration layer.
