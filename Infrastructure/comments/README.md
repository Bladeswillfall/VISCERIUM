...
Account decoration deliberately stays **outside Preact-owned child markup**. The shell writes only inert `data-vc-*` attributes onto existing username/timestamp elements and CSS renders the visible `UID · Role` line with generated content. It does not append custom child nodes that Preact could reconcile away. The existing child-list `MutationObserver` remains dedicated solely to removing dynamically-created upload controls; account code is never called from that observer and does not listen to `input`, `keydown` or other typing events. Identity decoration runs during a bounded startup burst, after comment submission, after low-frequency clicks, and after explicit parent theme messages so runtime editor updates remain upstream-owned.
