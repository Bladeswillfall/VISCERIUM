...
The same shell provides the VISCERIUM presentation layer without forking Remark42. It remaps Remark42's existing colour variables to the Codex light/dark palette and the CITADEL, SMOG, NEARSIGHT and ENTROPY accents inferred from the canonical `/eras/<era>/...` article URL. Theme switching is deliberately CSS-driven from Remark42's own literal direct-child `.light`/`.dark` root classes; the override does not observe class mutations and therefore does not enter Remark42's editor/render hot path when a user types. Native Remark42 composer, link and button geometry remains upstream-owned; the override recolours those controls rather than restyling them. VISCERIUM-owned structural additions are limited to the era-coloured comment root rail, borderless avatars, compact identity metadata and the Staff role tag.

The DOM observer is intentionally restricted to `childList` additions. Newly-added subtrees are batched to the next animation frame and inspected only for upload controls, the exact top-level comment form, and newly-rendered comments. It does not watch attributes, class changes, textarea values or theme state. This is a compatibility requirement: typing, previewing, replying, editing and theme switching must not cause the VISCERIUM decoration layer to re-run globally.
