...
const supported = new Set(['citadel']);
...
for (const file of files) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!supported.has(eraId)) {
      output.push(warning(`No primer is registered for '${eraId}'.`));
      continue;
    }

    used = true;
    output.push(`<EraPrimer eraId=${JSON.stringify(eraId)} />`);
  }

  if (!used) {
    const content = output.join('\n');
    await fs.writeFile(file, matter.stringify(content, parsed.data));
    continue;
  }
  ...
  console.log(`Expanded era primer content in ${path.relative(docsDir, outFile)}`);
}
