...
import { computeSafeFloatingPosition, getVisualViewportRect } from '../ui/floating-position.mjs';
...
export function installTimelineHovercard(root, dataset) {
  ...
  const position = () => {
    ...
    const result = computeSafeFloatingPosition({
      ...
      floatingRect: tooltip.getBoundingClientRect(),
      viewportRect: getVisualViewportRect(window),
      safeArea: 8,
      gap: 10,
